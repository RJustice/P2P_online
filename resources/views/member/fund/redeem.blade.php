        ...
        <ul>
          <li class="clearfix"><label>选择赎回的产品:</label>
            <select name="deal_order_id" id="deal-order-select">
                ...
                <?php 
                    ...
                    $nowDays = $diff2->days;
                 ?>
                <option value="{{ $dealOrder->getKey() }}">{{ $dealOrder->deal_title }} - 开始：{{ $dealOrder->create_date }} - {{ $dealOrder->total_price }}元</option>
                @endforeach
            </select>
          </li>
          <li class="clearfix"><label>产品详情：</label>
            <div style="float:left;" id="deal-order-info">
                <p>&nbsp;</p>
                <p><label class="d-label">理财产品：</label>{{ $dealOrder->deal_title }}</p>
                <p><label class="d-label">起始日期：</label>{{ $dealOrder->create_date }} ~ {{ $dealOrder->finish_date }}</p>
                <p><label class="d-label">投资金额：</label><span class="money">{{ $dealOrder->total_price }}</span></p>
                <p><label class="d-label">预期年利率：</label>{{ $dealOrder->deal_rate }}</p>
                <p><label class="d-label">预期到期收益：</label><span class="money">{{ number_format($dealOrder->deal_daily_returns * $days * $dealOrder->total_price / 10000,2) }}</span></p>
                <p><label class="d-label">今日赎回待收收益：</label><span class="money">{{ number_format($dealOrder->deal_daily_returns * $nowDays * $dealOrder->total_price / 10000,2) }}</span></p>                
            </div>
          </li>
          <li class="clearfix"><label>手续费：</label><span class="money">{{ number_format($dealOrder->total_price * 0.03,2) }}</span></li>
