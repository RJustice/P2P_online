...
<div class="tab-box pub-box resetphone-box invest-tc-box" style="width:720px">
    {!! Form::open(['route'=>'member.account.resetphone.two','class'=>'reset-phone-one','id'=>'reset-phone-one']) !!}

    <div class="me-invest change-pad">
        <div class="step" style="display: block;">
            <p class="tc"><img src="/images/step2.png"></p>
            <p class="p-invest"><span class="mar-left">验证原手机号码</span><span class="span02 fz-color">验证新手机号码</span><span class="span03">成功</span></p>
        </div>
        <div class=" form-box" style="display: block;">
            <ul>
                <li class="clearfix"><label>输入新手机号：</label><input type="text" placeholder="" id="new-tel-number" name="new_tel"><span class="error"></span></li>
                <li class="clearfix"><label>手机验证码：</label><input type="text" placeholder="" class="change-input-wid" id="new-tel-code" name="new_tel_code"><span class="getcode" id="re-sms">获取验证码</span><span id="send-sms-tip" class="error"></span></li>
            </ul>
            <p class="clearfix p-btn"><button class="a-btn" id="modifytel-twostep" href="javascript:void(0)">下一步</button></p>
        </div>
    </div>
    {!! Form::close() !!}
</div>
    ...
    $(document).ready(function(){
        ...
        $("#re-sms").on('click',function(){
            ...
            $.ajax({
                url : '{{ url('sms/test') }}',
                type : 'post',
                data : {_token:'{{ csrf_token() }}',new_tel:$("#new-tel-number").val()},
                dataType : 'json',
